def dry_setup
  ...
  require File.join(File.dirname(__FILE__), "../lib/model_driven")
  options = {
...
namespace :model_driven do
  ...
  desc "Generate models and their associations from uml class diagramm (DIA) or YAML"
  task :generate do
    raise "usage: rake model_driven:generate filename=<path_to_file>" unless ENV.include?("filename")
    options = dry_setup
    ModelDriven::Design.load(options).emit
  end
  ...
  task :yaml do
    ...
    options = dry_setup
    puts ModelDriven::Design.load(options).to_yaml
  end
